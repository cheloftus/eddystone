...
Usage for command "eurl":
    python eidtools.py eurl <url> <il> <scaler> <beacon_time_seconds>

    Generates an EURL for the provided URL using the given 16-bytes identity
    key, the scaler and the beacon time in seconds.
...
def PrintUri(desc, uri, encuri=None):
  """Print a URI in several formats including the encoded binary."""
  if encuri == None:
    encuri = encodeUri(uri)

  s = desc + ":\n"

  s += "  ASCII:  "
  if all("\x20" <= c < "\x80" for c in uri):
    s += uri + ""
  else:
    s += "[unprintable]"
  s += "\n"

  for i, c in enumerate(encuri):
    if i % 8 == 0:
      if i != 0:
        s += "\n"
      s += "  "
...
def GetAndPrintEurl(url, ik, scaler, beacon_time_seconds):
  """Return the EURL generated by the given parameters."""
  eid = GetAndPrintEid(ik, scaler, beacon_time_seconds)
  enceid = base64.b64encode(eid)
  encurl = encodeUri(url)
  enceid = enceid[:18 - len(encurl) - 1]
  eurl = "{0}?{1}".format(url, enceid)
  PrintUri("Ephemeral URL", eurl)
  return eurl
  
def encodeUri(uri):
  """encodeUri from taken from eddystone-url/implementations/linux-url-advertiser/advertise-url"""
  schemes = [
    "http://www.",
    "https://www.",
    "http://",
...
def main(command, args):
  if command == "registration":
    ...
    if len(args) != 4:
      Usage()
    url = args[0]
    ik = GetBinary(args[1], 16)
    PrintBinary("Identity Key", ik)
    scaler = int(args[2], 0)
    beacon_time_seconds = int(args[3], 0)
    GetAndPrintEurl(url, ik, scaler, beacon_time_seconds)
